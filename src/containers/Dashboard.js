...
export default class {
  ...
  handleShowTickets(e, bills, index) {
    ...
    if (state === -1) {
      ...
      // display bills for this status
      $(`#status-bills-container${this.index}`).html(
        cards(filteredBills(bills, getStatus(this.index)))
      );

      // add event listener on each bill of this status
      ...
      // put index in the list of open status
      this.openStatus.push(index);
    } else {
      // rotate arrow
      $(`#arrow-icon${this.index}`).css({ transform: "rotate(90deg)" });

      // remove bills for this status
      $(`#status-bills-container${this.index}`).html("");

      // remove event listener on each bill of this status
      filteredBills(bills, getStatus(this.index)).forEach((bill) => {
        $(`#open-bill${bill.id}`).off();
      });

      // remove index in the list of open status
      this.openStatus.pop(index);
    }
